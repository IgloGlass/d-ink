...
import { safeParseAuditEventV1 } from "../../../src/shared/contracts/audit-event.v1";

describe("AuditEventV1Schema", () => {
  const baseEvent = {
  ...
  it("accepts a valid system event", () => {
    const result = safeParseAuditEventV1(baseEvent);

    expect(result.success).toBe(true);
  });

  it("accepts a valid user event when actorUserId is present", () => {
    const result = safeParseAuditEventV1({
      ...baseEvent,
      actorType: "user",
  ...
  it("rejects timestamp without timezone offset", () => {
    const result = safeParseAuditEventV1({
      ...baseEvent,
  ...
  it("rejects non-UUID targetId values in V1", () => {
    const result = safeParseAuditEventV1({
      ...baseEvent,
