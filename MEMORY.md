...
- 2026-03-11: Added a first-class annual-report Playwright smoke script (`pnpm run test:annual-report:ui`) that validates the live localhost module layout, auto-signs through demo auth via the company landing flow, captures `output/playwright/annual-report-module.png`, and avoids the broken PowerShell `node`/`npx` shim path by running through the repo's `invoke-local-node-tool.ps1` helper.
- 2026-03-11: Locked an execution preference: when user feedback clearly implies a reusable improvement to tooling, testing, or workflow, codify it proactively in repo instructions or automation instead of treating it as one-off terminal knowledge.
- 2026-03-18: Completed four v1 bugfix/feature tickets:
  1. Launcher search — removed orphaned-workspace UUID display bug; tightened filter to company name/org-number only; fixed pre-existing test.
  2. Status transition UI — built `WorkspaceStatusTransitionV1` component (full 8-state machine, `filed→draft` reason input); wired into workspace dashboard hero and review panel sidebar; replaced the old 3-state stub in the review panel (which had an illegal `in_review→approved_for_export` direct jump).
  3. Review panel — fixed bug where `annual-report-analysis` was incorrectly excluded from the sidebar render condition, meaning the review panel never showed; comment/task creation was already implemented (memory was stale on this).
  4. `window.confirm()` — replaced all uses with `ConfirmModalV1` via an `onConfirmRequired` callback option on `useAnnualReportUploadControllerV1`; updated all call sites in both consumers.
- 2026-03-18: Tax submodule write-path gap identified (deferred). All 30 submodule UIs correctly READ from `getActiveTaxAdjustmentsV1` but have ZERO write path — no submodule calls `applyTaxAdjustmentsOverridesV1` or uses `useMutation`. `applyTaxAdjustmentsOverridesV1` exists in `workspace-api.ts` and works, just never called from submodule UI. Additionally, Submodule 01 (`TaxAdjSubmodule01GeneralClientInfoV1`) captures critical user inputs (periodiseringsfonder, underskott, ownership-change flag) in ephemeral local state that is never persisted — these feed modules 17 and 24. Next action when we return: build a self-contained `TaxDecisionOverrideV1` component (inline amount+reason form → `applyTaxAdjustmentsOverridesV1`), wire it into the 4 v1-critical submodules (non_deductible_expenses, representation_entertainment, depreciation_differences_basic, manual_review_bucket), and separately scope a backend endpoint for Submodule 01 general-client-info inputs.
